...
def newRestaurant():
    if request.method == 'POST':
        ...
        return render_template('newrest.html', uname=login_session['username'])
...
def newMenuItem(restaurant_id):
    ...
    if restaurant.user_id != login_session.get('user_id'):
        return "<script>function myFunction() {alert('You are not \
        authorized to add new item to this restaurant. Please create \
        your own restaurant in order to add.');}</script>\
        <body onload='myFunction()''>"
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if editedItem.user_id != login_session.get('user_id'):
        return "<script>function myFunction() {alert('You are not \
        authorized to edit this menu item. Please create \
        your own restaurant in order to edit.');}</script>\
        <body onload='myFunction()''>"
...
def Viewmenu(restaurant_id, menu_id):
    item = session.query(MenuItem).filter_by(id=menu_id).one()

    return render_template('viewmenu.html',
                           restaurant_id=restaurant_id,
                           menu_id=menu_id, item=item, uname=login_session['username'])
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if itemToDelete.user_id != login_session.get('user_id'):
        return "<script>function myFunction() {alert('You are not \
        authorized to delete this menu item. Please create \
